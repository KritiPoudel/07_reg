...
Seeding rate is numerical so we have one big plot, if we make factor we have different rates
...
nd <- nd %>% 
  mutate(mod1_yield_mgha = predict(mod1_int, nd,
                                   re.form = NA))
...
mod1_int_boots <- bootMer(mod1_int, 
                          predict.fun, 
                          nsim = 200) %>% #bootstrapping 200 times, but for research recommended 1000 times
  confint() %>%
  as.data.frame() %>%
...
#residuals change with the model, so assumptions should be checked based on residuals not the data alone
```
...
# Creating predictions
nd <- nd %>%
  mutate(mod3_yield_mgha = predict(mod3_quad, #prediction of mean of the line
                                   nd, 
                                   re.form = NA))

# Bootstrapping
mod3_quad_boots <- bootMer(mod3_quad, #error associated with the line associated with different level of seeding rate
                           predict.fun, 
                           nsim = 200) %>%
  confint() %>% #confidence interval by simulating it 200 times
  as.data.frame() %>%
...
ggplot(reg_dfw, 
  ...
  geom_ribbon(data = nd, aes(x = sr_ksha, 
              ...
              alpha = 0.5, 
              inherit.aes = FALSE)#+
  geom_point(data = nd, aes(x = sr_ksha, 
                             y= mod3_quad_lcl))
...
mod4_linp <- nlme(yield_mgha ~ SSlinp(sr_ksha,
                                      a, b, xs),# we need to estimate three things and SSlinp helps us to tell model they are estimation or the part of the same model
                  
                  data = reg_dfw,
                  random = list(rep = pdDiag(a + b+ xs ~ 1)),#because we are using random effects as well
                  fixed = list(a ~ 1,
                               b ~ 1,
                               xs ~ 1),
                  start = c(a = 0,
